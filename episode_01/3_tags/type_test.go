...
func TestUnitDecodeFrontendRequest(t *testing.T) {
	const jsonStream = `{"artifact-url": "s3://input/foo.tar.bz2", "output": "s3://output/", "instance-count": 2, "instance-type": "m4.large", "region": "us-east-1", "ami-id": "ami-daf4bfba"}`
	...
	for {
		...
		if m.AmiID != "ami-daf4bfba" {
			t.Fatalf("unexpected value: %s", m.AmiID)
		}
	}
}
